...
class NormalBreakpoint(gdb.Breakpoint):
    ...
    def stop(self):
        value = compass.get()
        # gdb.execute('stepi')
        set_float(ADDRESS_COMPASS, value)
        # jumpto(0x4790)
        log.write(f"Setting Compass value to {value} degree\n")
        print(f"Setting Compass value to {value} degree\n")
